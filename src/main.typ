...
#align(center)[#nonumber[= Abstract]]

#block(
  inset: (left: 12pt, y: 4pt),
  stroke: (left: 4pt + gray.lighten(50%)),
  [
...
All--atom membrane systems were, for fluid systems, constructed using the CHARMM--GUI membrane builder @lee_charmm-gui_2019. Gel-like membranes were built by hand from a SLipids pre-equilibrated patch @slipids_downloads. Symmetric bilayers composed of 100 lipids per leaflet were generated under periodic boundary conditions, resulting in lamellar multilayer systems separated by aqueous slabs.
...
The equations of motion were integrated using a 2 fs timestep during production runs, with all bonds involving hydrogen atoms constrained using the LINCS algorithm. Long-range electrostatic interactions were computed using the particle mesh Ewald (PME) method, with a real-space cutoff of 12$angstrom$. The same cutoff was applied to short-range van der Waals interactions.
...
Then, 35 configurations were extracted with a step size of $tilde$0.1 nm along this trajectory and then used as starting points for the umbrella sampling simulations, each of which was run for 100ns. The distributions obtained in each window were subsequently combined using the weighted histogram analysis method (WHAM) in order to reconstruct the unbiased PMF, since the harmonic potential introduces a bias.
...
Hydration profiles were computed by counting the number of water molecules in the vicinity of the polar headgroup, using a distance-based cutoff, typically of the order of 3.5 $angstrom$ --- usually used for studying structuring waters from proteins @biedermannova_structure_2015. Hydration provides a measure of solvent penetration at the membrane interface and is particularly relevant for characterizing this region, where polar interactions and hydrogen bonding play a significant role.
...
#figure(
  grid(
    ...
    image("rsrc/fig-piechart-synaptic.png", width: 111%),
  ),
...
Starting from bulk water, the PMF rapidly decreases as the ligand approaches the interface, indicating a thermodynamic driving force for membrane insertion, confirming that the membrane constitutes a favorable environment for the ligand. Interestingly, a very small energy barrier ($tilde$0.25 kcal$dot$mol$#text[]^(-1)$, that is $tilde$0.4 kT at physiological temperature) is observed close to the water-lipid interface. Although small compared to thermal fluctuation, it likely reflects the energetic cost associated with crossing the phosphate headgroup region and locally perturbing lipid packing in order to access the hydrophobic core of the bilayer.

Once this barrier is crossed, the free--energy decreases toward a pronounced minimum of $tilde -11$ kcal$dot$mol$#text[]^(-1)$ located inside the membrane, in good agreement with the density profiles and orientational analyses. Finally, the PMF rises again when approaching the bilayer center, usually depleted in lipids. In practice, this free--energy increase should act as a barrier against spontaneous flip--flop events on accessible simulation timescales.

== Structural and mechanical response

We next examined how the membrane itself responds to ligand incorporation. To this end, we analyzed the area per lipid, the deuterium order parameter and the local hydration environment of the phosphate, which together should provide complementary information on membrane packing, ordering and interfacial organization. In mixed membranes, order parameter analyses were restricted to the host lipids (DOPC or DPPC). Indeed, the relatively small number of SAPC or SDPC molecules in the simulated patches did not allow statistically robust profiles to be extracted for the polyunsaturated species themselves.
